<?php
namespace App\Exceptions\WxPay;
class WxComPay
{
	// 证书
...
// 	private $rootca='';
	 ...
	 */
	public function setCert($apiclient_cert)
	{
		$this->apiclient_cert =getcwd().$apiclient_cert;
	}
	public function setKey($apiclient_key)
	{
		$this->apiclient_key =getcwd().$apiclient_key;
	}
	
// 	public function setCA($rootCA)
// 	{
// 		$this->rootca = dirname ( __FILE__ ) . "/cacert/".$rootCA;
// 	}
	/**
	 * 错误反馈
	 * 
	...
	public function RedBag()
	{
		if (! $this->inited ())
		{
			echo '验证失败'.$this->error.$this->amount;
			exit();
			return;
		}
	...
	private function license()
	{
		if (! $this->apiclient_cert)
			$this->apiclient_cert =APP_PATH . "Data/cert/". "apiclient_cert.pem";
		if (! $this->apiclient_key)
			$this->apiclient_key = APP_PATH . "Data/cert/" . "apiclient_key.pem";
// 		if (! $this->rootca)
// 			$this->rootca = dirname ( __FILE__ ) . "/cacert/rootca.pem";
	}
...
// 		curl_setopt ( $ch, CURLOPT_SSLKEYTYPE, 'PEM' );
// 		curl_setopt ( $ch, CURLOPT_CAINFO, $this->rootca); // CA根证书（用来验证的网站证书是否是CA颁布）
